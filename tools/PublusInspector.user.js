...
// @version      4.0.0
// @description  Inspector soi ma trận ACCESS PUBLUS / NFBR cho BookWalker và Pixiv Comic Store Tankobon.
// @author       anonymous & AI
...
(function publusInspector() {
  ...
  // 1. TÁCH BIỆT HOOK THEO TỪNG TRANG TRONG MAIN-WORLD
  try {
    WIN.eval(`
      (function() {
        ...
        function hookNFBR() {
          ...
          for (var key in proto) {
            ...
            // Pixiv Store: lọc hàm 5 tham số; BookWalker: hook toàn bộ các hàm vẽ
  ...
  function getNFBRRuntime(targetWin = WIN) {
    try {
      ...
      let obj = null;
      if (isBookWalker) {
        obj = a6G.Initializer?.T1V || a6G.Initial?.T1V;
      } else {
        obj = a6G.Initializer?.F7F || a6G.Initial?.F7F;
      }
      ...
      onPreview: async (pNo, onSuccess, onError) => {
        try {
          ...
          onSuccess({
            ...
            sharpCanvas: res.sharpCanvas, rawCanvas: res.rawCanvas, img: res.rawCanvas,
            rawExt: "JPG"
          }, pNo);
