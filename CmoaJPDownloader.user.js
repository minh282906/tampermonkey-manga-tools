...
(function cmoaUniversalDownloader() {
  ...
  function saveJpegPref(val) {
    try {
      WIN.localStorage.setItem("cmoa-dl:convert-jpeg", val ? '1' : '0');
    } catch {}
  }
  ...
  function getCidPrefix() {
    ...
    return match ? match[1] : "Cmoa_Episode";
  }
  ...
  function createUI() {
    if (state.ui || !DOC.body || DOC.getElementById("cmoa-dl-panel")) return;

    const PANEL_WIDTH = 220;
    const TAB_WIDTH = 14;
    let isCollapsed = localStorage.getItem("cmoa-dl:collapsed") === '1';

    const panel = DOC.createElement("div");
    panel.id = "cmoa-dl-panel";
    panel.style.cssText = [
      "all:initial",
      "position:fixed",
      "right:0px",
    ...
    collapsedStrip.onmouseenter = () => { collapsedStrip.style.background = "#f97316"; };
    collapsedStrip.onmouseleave = () => { collapsedStrip.style.background = "#ea580c"; };
    ...
    collapseBtn.onmouseenter = () => { collapseBtn.style.background = "#f97316"; };
    collapseBtn.onmouseleave = () => { collapseBtn.style.background = "#ea580c"; };

    const title = DOC.createElement("div");
    title.textContent = "Cmoa Downloader";
    ...
    jpgInput.style.cssText = "all:initial;appearance:auto;width:14px;height:14px;accent-color:#ea580c;cursor:pointer;";
    ...
    track.style.cssText = "all:initial;display:block;height:6px;overflow:hidden;border-radius:3px;background:#431407;margin-top:6px;";
    ...
    function setCollapsedState(collapsed) {
      ...
      localStorage.setItem("cmoa-dl:collapsed", isCollapsed ? '1' : '0');
  ...
  async function startDownload() {
    ...
    try {
      ...
      console.error("[cmoa-dl] Download failed:", err);
    } finally {
