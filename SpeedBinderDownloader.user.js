...
(function yanmagaCmoaUniversalDownloader() {
  'use strict';
  ...
  function saveJpegPref(val) {
    try {
      WIN.localStorage.setItem("speedbinb-dl:convert-jpeg", val ? '1' : '0');
    } catch {}
  }
  ...
  async function createSilentFrame(timeoutMs = 45000) {
    ...
    DOC.getElementById("speedbinb-dl-silent-frame")?.remove();

    const frame = DOC.createElement("iframe");
    frame.id = "speedbinb-dl-silent-frame";
    ...
    const workers = Array(Math.min(limit, tasks.length)).fill(0).map(async () => {
      while (index < tasks.length) {
        ...
        try {
          ...
          console.error(`[speedbinb-dl] Lỗi trang ${currentIndex + 1}:`, err);
